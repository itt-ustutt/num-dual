...
pub trait DualNumRef<T>: DualNum<T> + for<'r> DualNumOps<&'r Self> {}
impl<T, D> DualNumRef<T> for D where D: DualNum<T> + for<'r> DualNumOps<&'r Self> {}
...
macro_rules! impl_dual_num_float {
    ($float:ty) => {
        impl DualNum<$float> for $float {
            ...
            // fn scale(&self, t: $float) -> Self {
            //     self * t
            // }

            // fn unscale(&self, t: $float) -> Self {
            //     self / t
            // }
...
//         fn helmholtz_energy<T>(&self, temperature: &T, volume: &T, moles: &Array1<T>) -> T
//         where
//             T: DualNumRef<f64>,
//             for<'a> &'a T: DualRefNum<f64, T>;
//     }
//
