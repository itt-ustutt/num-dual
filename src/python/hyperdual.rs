                ...
                match x2 {
                    None => {
                        if let Ok(x) = x1.extract::<f64>() {
                            return Ok(PyCell::new(py, PyDual2_64::from(Dual2_64::derivative(x)))?.to_object(py));
                        };
                        if let Ok(x) = x1.extract::<PyDual64>() {
                            return Ok(PyCell::new(py, PyDual2Dual64::from(Dual2::derivative(x.into())))?.to_object(py));
                        };
                        if let Ok([x]) = x1.extract::<[f64; 1]>() {
                            let py_vec = vec![PyCell::new(py, PyDual2_64::from(Dual2_64::derivative(x)))?];
                            return Ok(py_vec.to_object(py));
                        };
                        $(
                            if let Ok(x) = x1.extract::<[f64; $n]>() {
                                let arr = SVector::from(x).map(Dual2Vec64::derivative);
                    ...
                    Some(x2) => {
                        ...
                        //         let arr2 = SVector::new_vec(x2).map(HyperDualVec64::from).derive2();
                        ...
                        //         let arr1 = SVector::new_vec(x1).map(HyperDualVec64::from).derive1();
...
impl_derive!([
    ...
    (PyHyperDual64_2_2, 2, 2) //     (PyHyperDual64_2_3, 2, 3),
