...
pub trait BesselDual: DualNum<Primitive = f64> + Copy {
...
impl<T: DualNum<Primitive = f64> + Copy> BesselDual for T {}
...
fn polevl<T: DualNum + Copy>(x: T, coef: &[T::Primitive]) -> T {
    coef.iter()
        .skip(1)
        .fold(T::from(coef[0]), |acc, &c| acc * x + c)
}

fn p1evl<T: DualNum + Copy>(x: T, coef: &[T::Primitive]) -> T {
