...
macro_rules! impl_to_primitive {
    ($ty:ty, $to:ident) => {
        ...
        fn $to(&self) -> Option<$ty> {
            if self.eps1.is_zero() && self.eps2.is_zero() && self.eps1eps2.is_zero() {
                self.re.$to()
                } else { None }
        }
    }
...
// Returns None if HyperDual part is non-zero
impl<T: ToPrimitive + Num> ToPrimitive for HyperDual<T> {
...
impl<T: FromPrimitive + Num> FromPrimitive for HyperDual<T> {
...
impl<T: NumCast + Num> NumCast for HyperDual<T> {
...
impl<T, U> AsPrimitive<U> for HyperDual<T>
where
    T: AsPrimitive<U>,
    U: 'static + Copy,
{
